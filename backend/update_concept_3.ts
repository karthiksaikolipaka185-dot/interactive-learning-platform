...
async function main() {
    ...
    // We assume Exercise 1 is 'Exercise 8.1: Trigonometric Ratios' (sequenceOrder: 1)
    // and Concept 3 is sequenceOrder: 3 within that.

    // First find the exercise to be sure
    ...
    await prisma.subtopic.update({
        ...
        data: {
            ...
            description: 'Angle-Wise Behavior & Value Patterns (Cheat Sheet 3)' // Updating description too for consistency
        }
    });
    console.log("Updated Title.");

    // 3. Delete old ContentBlocks (and cascading Checkpoints if configured, but let's be explicit)
    // Prisma's onDelete: Cascade might handle it, but safely deleting blocks is better.
    // We need to find blocks first to delete their checkpoints if not cascaded.
    // The schema has `Checkpoint? @relation(fields: [checkpointId], references: [id])` on ContentBlock.
    // Usually we delete ContentBlock, and if we want to delete Checkpoint we might need to do it separately 
    // depending on the relation direction. Here ContentBlock has `checkpointId`.
    // Actually, `Checkpoint` is the one that might be orphaned if not careful, 
    // but the relation is `ContentBlock` -> `Checkpoint` (one-to-one optional).
    // Let's delete ContentBlocks.
...
Think of **cos** as a skier going DOWN ⛷️.`,
    ...
    await prisma.contentBlock.create({
        data: {
            ...
            contentType: 'markdown',
            sequenceOrder: 2,
            contentData: '', // Just a question trigger
...
**Pro Tip**: If you calculate sin θ = 1.5, check your math. It's wrong! ❌`,
    ...
    console.log("New Content Inserted Successfully for Concept 3.");
}
